...
L<RPC::ExtDirect::Server>.

RPC::ExtDirect::Client uses L<HTTP::Tiny> for the transport layer.
...
Instance method. Returns an L<RPC::ExtDirect::API> derived object with
L<Ext.Direct API|RPC::ExtDirect::Intro/API> declaration published by
the server.

Accepts one positional argument, API type, that can be either
...
in scalar context, but doing that is no longer recommended.
...
and adds them to the L<RPC::ExtDirect::Config> instance it uses:
